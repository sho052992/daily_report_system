...
public class EmployeeValidator {
     ...
     * @param service 呼び出しもserviceクラスのインスタンス
     * @param ev EmployeeViewのインスタンス
     * @param codeDuplicateCheckFlag 社員番号の重複チェックを実施するかどうか（実施する：true 実施しない：false)
     * @param passwordeCheckFlag パスワードの入力チェックを実施するかどうか（実施する：true 実施しない:false)
     * @return エラーのリスト
     */
    ...
    private static long isDuplicateEmployee(EmployeeService service,String code) {
        ...
        long employeesCounte = service.countByCode(code);
        return employeesCounte;
    }
    /**
     ...
     * @param password パスワードｆ
     * @param passwordCheckFlag パスワードの入力チェックを行うかどうか（実施する：true,実施しない:false)
     * @return エラーメッセージ
     */
